...
static PII_PATTERNS: &[(&str, &str, &str)] = &[
    ...
    ("credit_card", r"\b(?:4[0-9]{12}(?:[0-9]{3})?|5[1-5][0-9]{14}|3[47][0-9]{13}|6(?:011|5[0-9]{2})[0-9]{12})\b", "Credit card number detected"),
    // Email address
    ("email",       r"\b[A-Za-z0-9._%+\-]+@[A-Za-z0-9.\-]+\.[A-Za-z]{2,}\b",  "Email address detected"),
    // Phone: US formats
    ("phone",       r"\b(\+?1[\s.-]?)?\(?\d{3}\)?[\s.\-]\d{3}[\s.\-]\d{4}\b", "Phone number detected"),
    // Passport (generic)
    ("passport",    r"\b[A-Z]{1,2}[0-9]{6,9}\b",                               "Passport number detected"),
];
        ...
        if let Ok(re) = Regex::new(pattern) {
            if re.is_match(text) {
                return Some(Verdict::block(
                    ...
                    format!("{} — sending PII to Claude API is blocked by policy", description),
                ));
            }
        }
    }
