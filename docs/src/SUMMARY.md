# Summary

[Introduction](introduction.md)

# Get Started

- [Installation](getting-started/installation.md)
- [Quick Start](getting-started/quickstart.md)
- [How It Works](getting-started/how-it-works.md)

# Configuration

- [rules.json Reference](configuration/rules-yaml.md)
- [Bash Protection](configuration/bash.md)
- [File Path Protection](configuration/file-paths.md)
- [PII Detection](configuration/pii.md)
- [Prompt Injection](configuration/injection.md)
- [URL Blocking](configuration/urls.md)
- [Webhook Integration](configuration/webhook.md)

# Hooks

- [Overview](hooks/overview.md)
- [UserPromptSubmit](hooks/user-prompt-submit.md)
- [PreToolUse](hooks/pre-tool-use.md)
- [PostToolUse](hooks/post-tool-use.md)
- [Stop](hooks/stop.md)

# Detectors

- [Overview](detectors/overview.md)
- [Command Detector](detectors/commands.md)
- [Path Detector](detectors/paths.md)
- [PII Detector](detectors/pii.md)
- [Secrets Detector](detectors/secrets.md)
- [Injection Detector](detectors/injection.md)
- [URL Detector](detectors/urls.md)

# Reference

- [Architecture](reference/architecture.md)
- [CLI Reference](reference/cli.md)
- [Audit Log](reference/audit-log.md)
- [Console](reference/console.md)
- [Contributing](reference/contributing.md)
